...
pub struct RendezvousServer {
    ...
    inner: Arc<Inner>,
    conn_map: Arc<Mutex<HashMap<SocketAddr, Arc<Mutex<(Sink, String)>>>>>,
...
impl RendezvousServer {
                ...
                Some(rendezvous_message::Union::RelayResponse(mut rr)) => {
                    ...
                    if !rr.relay_server.is_empty() {
                        if self.is_lan(addr_b) {
                            ...
                            rr.relay_server = self.get_relay_server(self.to_ip(addr), self.to_ip(addr_b)).0;
                        }
                    }
                ...
                Some(rendezvous_message::Union::RegisterPk(rk)) => {
                    ...
                    match response {
                        ...
                        Ok(res) => {
                            ...
                            Self::send_to_sink(sink, msg_out).await;
                            if let Some(sink) = sink.take() {
                                self.conn_map.lock().await.insert(try_into_v4(addr), Arc::new(Mutex::new((sink, rk.id))));
                            }
                            return true;
                        }
                    }
    ...
    async fn handle_punch_hole_request(
        ...
        if let Some(peer) = self.pm.get(&id).await {
            ...
            let (mut relay_server, force_relay) = self.get_relay_server(self.to_ip(addr), self.to_ip(peer_addr));
            if ALWAYS_USE_RELAY.load(Ordering::SeqCst) || (peer_is_lan ^ is_lan || force_relay) {
                if peer_is_lan {
                    // https://github.com/rustdesk/rustdesk-server/issues/24
    ...
    async fn send_to_tcp(&mut self, msg: RendezvousMessage, addr: SocketAddr) {
        ...
        let sink_arc = self.conn_map.lock().await.get(&addr_v4).cloned();
    ...
    fn get_relay_server(&self, pa: IpAddr, pb: IpAddr) -> (String, bool) {
        if self.relay_servers.is_empty() {
            return ("".to_owned(), false);
        } else if self.relay_servers.len() == 1 {
            return (self.relay_servers[0].clone(), false);
        }
                    ...
                    if let Ok(a) = rs.parse::<IpAddr>() {
                        if let Some(rs) = fds.next() {
                            if let Ok(b) = rs.parse::<IpAddr>() {
                                res = format!("{:?}", self.get_relay_server(a, b).0);
                            }
                        } else {
                            res = format!("{:?}", self.get_relay_server(a, a).0);
                        }
                    }
                }
            }
    ...
    async fn handle_listener_inner(
            ...
            while let Ok(Some(Ok(msg))) = timeout(REG_TIMEOUT as u64, b.next()).await {
                if let tungstenite::Message::Binary(bytes) = msg {
                    ...
                    if bytes.is_empty() {
                        ...
                        // Update last_reg_time for this peer
                        let addr_v4 = try_into_v4(addr);
                        if let Some(sink) = self.conn_map.lock().await.get(&addr_v4) {
        ...
        self.conn_map.lock().await.remove(&try_into_v4(addr));
